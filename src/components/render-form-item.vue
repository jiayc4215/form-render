...
// 弃用
const enableWhenStatus = computed(() => getEnableWhenStatus(props.data.enableWhen, props.data.value))
// 处理组件的显示与隐藏
...
const multipleValue = computed(() => {
  ...
  return multipleSelectValue.map(val => (props.options.find(op => op.value === val) || {}).label).join()
})
// 处理服务器获取 options
const makingRequest = (remoteConfig, query) => {
  const isOptionsCase = ["select", "checkbox-group", "radio-group"].indexOf(props.data.type) > -1
  const {
...
watch(
  ...
  (url, oldV) => {
    ...
    const request = props.data.remote.request || (() => axios.get(url).then(resp => resp.data))
...
const remoteMethod = query => {
  if (_get(props.data, "type") === "select" && _get(props.data, "el.filterable") && _get(props.data, "el.remote")) {
