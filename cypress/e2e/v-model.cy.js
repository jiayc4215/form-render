...
describe("测试 v-model 示例", function () {
  ...
  it("基础用例", function () {
    ...
    // // 修改日期
    // cy.$getFormItemInput("date").eq(1).click()
    // // 假设点击后弹出日期面板，选择一个日期，或者通过输入修改
    // // 为了保持原有用例逻辑（修改为 03），我们还是采用输入的方式，但先 clear
    // cy.$getFormItemInput("date").eq(1).clear()
    // cy.$getFormItemInput("date").eq(1).type("2019-01-03{enter}")
